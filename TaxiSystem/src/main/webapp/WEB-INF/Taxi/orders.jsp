...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark justify-content-between">
    ...
    <ul class="navbar-nav mr-auto">
        <li class="nav-item">
            <a class="nav-link active" href="Controller?method=dispatcher&action=getTaxiOrders">$$$Заказы</a>
        </li>
        <li class="nav-item">
                    ...
                    <td><c:choose>
                        <c:when test = "${order.orderStatus eq 'processed'}">
                            <form action="Controller" method="post">
                                ...
                                <button class="btn btn-light" type="submit">$$$Подтвердить</button>
                            </form>
                            <form action="Controller" method="post">
                                <input type="hidden" name="method" value = "dispatcher"/>
                                <input type="hidden" name="action" value="rejectOrder"/>
                                <input type="hidden" name="orderId" value="${order.orderId}"/>
                                <button class="btn btn-light" type="submit">$$$Отклонить</button>
                            </form>
                        </c:when>
                        <c:when test="${order.orderStatus eq 'accepted'}">
                            <button class="btn btn-dark" onclick="setCoord(${order.sourceCoordinate},${order.destinyCoordinate})">$$$Показать на карте</button>
                        </c:when>
                        <c:when test="${order.orderStatus eq 'rejected'}">
                            <button class="btn btn-secondary" disabled>$$$Заказ отклонен</button>
                        </c:when>
                        <c:when test="${order.orderStatus eq 'completed'}">
                            <button class="btn btn-secondary" disabled>$$$Заказ выполнен</button>
                        </c:when>
                    </c:choose></td>
                </tr>
            </c:forEach>
